...
function getSaveElements() {
  ...
  var saveButton = saveLink ? saveLink.querySelector("button") : null;
  var saveLabel = saveButton ? saveButton.querySelector(".hide-smallsc") : null;
  var saveStatus = document.getElementById("save-status");
  return { saveLink: saveLink, saveButton: saveButton, saveLabel: saveLabel, saveStatus: saveStatus };
}

function setSaveStatus(message, isError) {
  var els = getSaveElements();
  if (!els.saveStatus) {
    return;
  }
  els.saveStatus.textContent = message || "";
  els.saveStatus.style.color = isError ? "#f87171" : "#34d399";
}
...
function refreshSaveButtonState() {
  ...
  var hasFile = Boolean(param("file"));
  els.saveButton.disabled = !hasFile;
  els.saveButton.style.opacity = hasFile ? "1" : "0.6";
  els.saveButton.style.cursor = hasFile ? "pointer" : "not-allowed";
  if (!hasFile) {
    setSaveStatus("", false);
  }
}
...
async function save2() {
  ...
  if (!window.editor) {
    setSaveStatus("Editor is still loading. Try again in a moment.", true);
    return;
  }
  ...
  try {
    ...
    let myJson = await myObject.json();
    console.log(myJson);
    if (myJson && myJson.data === "success") {
      window.value = code;
      setSaveStatus("Saved successfully.", false);
    } else {
      setSaveStatus((myJson && (myJson.err || myJson.error || myJson.data)) || "Save failed.", true);
    }
  } catch (err) {
    setSaveStatus("Save failed due to a network/runtime error.", true);
  } finally {
    if (els.saveLabel) {
      els.saveLabel.textContent = "Save";
    }
    refreshSaveButtonState();
  }
}
...
async function guildData(dir) {
  ...
  for (let i = 0; i < json.length; i++) {
    ...
    if (json[i].type == "file") {
      code += `<a href="editor.html?file=${json[i].path}"><div class="text-white rounded-md pl-2 py-1 bg-black m-1">${somed}</div></a>`
    }
    if (json[i].type == "dir") {
      code = `<a href="editor.html?folder=${json[i].path}"><div class="text-white rounded-md pl-2 py-1 bg-black m-1">./${somed}/</div></a>` + code
    }
  }
...
function back() {
  ...
  if (!param("folder")) {
    ...
    window.location.href = "editor.html?folder=" + param("folder").replace(f, "")
  }
}
function unnewf() {
